...
if ($_SERVER['REQUEST_METHOD'] === 'POST'){
    ...
    $booking_sql = "SELECT room_id, num_rooms FROM bookings WHERE id = $cancel_id AND user_id = $user_id AND status NOT IN ('cancelled','checked_out')";
    ...
    header("Location: booking_list_fix.php");
    exit();
}
...
// Auto-checkout for paid bookings that reached checkout date
$sql_checkout = "
    SELECT id, room_id, num_rooms 
    FROM bookings 
    WHERE user_id = $user_id 
      AND status = 'paid' 
      AND check_out <= '$today'
...
if ($result_checkout && $result_checkout->num_rows > 0) {
    while ($row = $result_checkout->fetch_assoc()) {
        ...
        $conn->query("UPDATE bookings SET status = 'checked_out' WHERE id = $booking_id");
